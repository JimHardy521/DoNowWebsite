const gcseQuestions = [

    /* =========================
       FOUNDATION
       ========================= */
    
    {
      topic: "Decimals",
      level: "Foundation",
      question: "Work out \\(3.6 + 0.48\\).",
      solution: "Align the decimal points: \\(3.60 + 0.48 = 4.08\\)."
    },
    {
      topic: "Decimals",
      level: "Foundation",
      question: "Calculate \\(5.2 \\times 0.4\\).",
      solution: "Multiply ignoring decimals: \\(52 \\times 4 = 208\\). Adjust decimals: \\(2.08\\)."
    },
    {
      topic: "Decimals",
      level: "Foundation",
      question: "Work out \\(7.5 \\div 0.5\\).",
      solution: "Dividing by 0.5 is the same as multiplying by 2: \\(7.5 \\times 2 = 15\\)."
    },
    {
      topic: "Decimals",
      level: "Foundation",
      question: "Round \\(4.387\\) to 2 decimal places.",
      solution: "The third decimal digit is 7, so round up: \\(4.39\\)."
    },
    {
      topic: "Decimals",
      level: "Foundation",
      question: "Write \\(0.625\\) as a fraction in its simplest form.",
      solution: "\\(0.625 = \\frac{625}{1000} = \\frac{5}{8}\\)."
    },
    
    {
      topic: "Fractions",
      level: "Foundation",
      question: "Simplify \\(\\frac{12}{18}\\).",
      solution: "Divide numerator and denominator by 6: \\(\\frac{2}{3}\\)."
    },
    {
      topic: "Fractions",
      level: "Foundation",
      question: "Work out \\(\\frac{3}{4} + \\frac{1}{8}\\).",
      solution: "Convert to eighths: \\(\\frac{6}{8} + \\frac{1}{8} = \\frac{7}{8}\\)."
    },
    {
      topic: "Fractions",
      level: "Foundation",
      question: "Calculate \\(\\frac{5}{6} \\times \\frac{3}{10}\\).",
      solution: "\\(\\frac{15}{60} = \\frac{1}{4}\\)."
    },
    {
      topic: "Fractions",
      level: "Foundation",
      question: "Find \\(\\frac{2}{3}\\) of 24.",
      solution: "One third of 24 is 8, so two thirds is 16."
    },
    {
      topic: "Fractions",
      level: "Foundation",
      question: "Write \\(1\\frac{1}{4}\\) as an improper fraction.",
      solution: "\\(1\\frac{1}{4} = \\frac{5}{4}\\)."
    },
    
    {
      topic: "Standard Form",
      level: "Foundation",
      question: "Write \\(3,500,000\\) in standard form.",
      solution: "\\(3.5 \\times 10^6\\)."
    },
    {
      topic: "Standard Form",
      level: "Foundation",
      question: "Write \\(0.0042\\) in standard form.",
      solution: "\\(4.2 \\times 10^{-3}\\)."
    },
    {
      topic: "Standard Form",
      level: "Foundation",
      question: "Calculate \\((2 \\times 10^4)(3 \\times 10^2)\\).",
      solution: "\\(6 \\times 10^6\\)."
    },
    {
      topic: "Standard Form",
      level: "Foundation",
      question: "Calculate \\(6 \\times 10^5 \\div 3 \\times 10^2\\).",
      solution: "\\(2 \\times 10^3\\)."
    },
    {
      topic: "Standard Form",
      level: "Foundation",
      question: "Which is larger: \\(5 \\times 10^4\\) or \\(4.8 \\times 10^4\\)?",
      solution: "\\(5 \\times 10^4\\) is larger."
    },
    
    {
      topic: "Solving Equations",
      level: "Foundation",
      question: "Solve \\(3x + 7 = 19\\).",
      solution: "\\(3x = 12\\), so \\(x = 4\\)."
    },
    {
      topic: "Solving Equations",
      level: "Foundation",
      question: "Solve \\(5x = 40\\).",
      solution: "\\(x = 8\\)."
    },
    {
      topic: "Solving Equations",
      level: "Foundation",
      question: "Solve \\(x - 6 = 11\\).",
      solution: "\\(x = 17\\)."
    },
    {
      topic: "Solving Equations",
      level: "Foundation",
      question: "Solve \\(2(x + 3) = 14\\).",
      solution: "\\(x + 3 = 7\\), so \\(x = 4\\)."
    },
    {
      topic: "Solving Equations",
      level: "Foundation",
      question: "Solve \\(7x - 5 = 16\\).",
      solution: "\\(7x = 21\\), so \\(x = 3\\)."
    },
    
    {
      topic: "Expanding and Simplifying Expressions",
      level: "Foundation",
      question: "Expand \\(3(x + 4)\\).",
      solution: "\\(3x + 12\\)."
    },
    {
      topic: "Expanding and Simplifying Expressions",
      level: "Foundation",
      question: "Simplify \\(5a + 2a\\).",
      solution: "\\(7a\\)."
    },
    {
      topic: "Expanding and Simplifying Expressions",
      level: "Foundation",
      question: "Expand \\(2(3x - 5)\\).",
      solution: "\\(6x - 10\\)."
    },
    {
      topic: "Expanding and Simplifying Expressions",
      level: "Foundation",
      question: "Simplify \\(4y - 3y + 7\\).",
      solution: "\\(y + 7\\)."
    },
    {
      topic: "Expanding and Simplifying Expressions",
      level: "Foundation",
      question: "Expand and simplify \\(x + 2(x + 5)\\).",
      solution: "\\(3x + 10\\)."
    },
    
    /* =========================
       CROSSOVER
       ========================= */
    
    {
      topic: "Expanding Quadratic Expressions",
      level: "Crossover",
      question: "Expand \\((x + 4)(x + 3)\\).",
      solution: "\\(x^2 + 7x + 12\\)."
    },
    {
      topic: "Expanding Quadratic Expressions",
      level: "Crossover",
      question: "Expand \\((x - 5)(x + 2)\\).",
      solution: "\\(x^2 - 3x - 10\\)."
    },
    {
      topic: "Expanding Quadratic Expressions",
      level: "Crossover",
      question: "Expand \\((2x + 1)(x + 4)\\).",
      solution: "\\(2x^2 + 9x + 4\\)."
    },
    {
      topic: "Expanding Quadratic Expressions",
      level: "Crossover",
      question: "Expand \\((x - 3)^2\\).",
      solution: "\\(x^2 - 6x + 9\\)."
    },
    {
      topic: "Expanding Quadratic Expressions",
      level: "Crossover",
      question: "Expand \\((2x - 5)(x - 1)\\).",
      solution: "\\(2x^2 - 7x + 5\\)."
    },
    
    {
      topic: "Factorising and Solving Equations",
      level: "Crossover",
      question: "Solve \\(x^2 + 5x = 0\\).",
      solution: "\\(x(x + 5) = 0\\), so \\(x = 0\\) or \\(x = -5\\)."
    },
    {
      topic: "Factorising and Solving Equations",
      level: "Crossover",
      question: "Solve \\(x^2 - 9 = 0\\).",
      solution: "\\((x - 3)(x + 3) = 0\\), so \\(x = \\pm3\\)."
    },
    {
      topic: "Factorising and Solving Equations",
      level: "Crossover",
      question: "Solve \\(x^2 + 7x + 10 = 0\\).",
      solution: "\\((x + 5)(x + 2) = 0\\), so \\(x = -5, -2\\)."
    },
    {
      topic: "Factorising and Solving Equations",
      level: "Crossover",
      question: "Solve \\(2x^2 - 8x = 0\\).",
      solution: "\\(2x(x - 4) = 0\\), so \\(x = 0\\) or \\(x = 4\\)."
    },
    {
      topic: "Factorising and Solving Equations",
      level: "Crossover",
      question: "Solve \\(x^2 - 4x = 0\\).",
      solution: "\\(x(x - 4) = 0\\), so \\(x = 0\\) or \\(x = 4\\)."
    },
    
    {
      topic: "Solving Simultaneous Equations",
      level: "Crossover",
      question: "Solve \\(x + y = 10\\) and \\(x - y = 2\\).",
      solution: "Adding gives \\(2x = 12\\), so \\(x = 6\\). Then \\(y = 4\\)."
    },
    {
      topic: "Solving Simultaneous Equations",
      level: "Crossover",
      question: "Solve \\(x + y = 7\\) and \\(2x - y = 8\\).",
      solution: "Add equations to get \\(3x = 15\\), so \\(x = 5\\), \\(y = 2\\)."
    },
    {
      topic: "Solving Simultaneous Equations",
      level: "Crossover",
      question: "Solve \\(x - y = 1\\) and \\(x + y = 9\\).",
      solution: "Add to get \\(2x = 10\\), so \\(x = 5\\), \\(y = 4\\)."
    },
    {
      topic: "Solving Simultaneous Equations",
      level: "Crossover",
      question: "Solve \\(2x + y = 11\\) and \\(x + y = 8\\).",
      solution: "Subtract to get \\(x = 3\\), then \\(y = 5\\)."
    },
    {
      topic: "Solving Simultaneous Equations",
      level: "Crossover",
      question: "Solve \\(3x + y = 13\\) and \\(x + y = 7\\).",
      solution: "Subtract to get \\(2x = 6\\), so \\(x = 3\\), \\(y = 4\\)."
    },
    
    {
      topic: "Simplifying Indices",
      level: "Crossover",
      question: "Simplify \\(x^3 \\times x^4\\).",
      solution: "\\(x^7\\)."
    },
    {
      topic: "Simplifying Indices",
      level: "Crossover",
      question: "Simplify \\(x^6 \\div x^2\\).",
      solution: "\\(x^4\\)."
    },
    {
      topic: "Simplifying Indices",
      level: "Crossover",
      question: "Simplify \\((x^3)^2\\).",
      solution: "\\(x^6\\)."
    },
    {
      topic: "Simplifying Indices",
      level: "Crossover",
      question: "Simplify \\(x^0\\).",
      solution: "\\(1\\)."
    },
    {
      topic: "Simplifying Indices",
      level: "Crossover",
      question: "Simplify \\(2x^2 \\times 3x^3\\).",
      solution: "\\(6x^5\\)."
    },
    
    /* =========================
       HIGHER
       ========================= */
    

   // =======================
// Surds – Simplifying
// =======================

{
  topic: "Surds - Simplifying",
  level: "Higher",
  question: "Simplify \\(\\sqrt{50}\\).",
  solution: "\\(\\sqrt{25 \\times 2} = 5\\sqrt{2}\\)."
},
{
  topic: "Surds - Simplifying",
  level: "Higher",
  question: "Simplify \\(\\sqrt{72}\\).",
  solution: "\\(\\sqrt{36 \\times 2} = 6\\sqrt{2}\\)."
},
{
  topic: "Surds - Simplifying",
  level: "Higher",
  question: "Simplify \\(\\sqrt{18} + \\sqrt{8}\\).",
  solution: "\\(3\\sqrt{2} + 2\\sqrt{2} = 5\\sqrt{2}\\)."
},
{
  topic: "Surds - Simplifying",
  level: "Higher",
  question: "Simplify \\(2\\sqrt{12}\\).",
  solution: "\\(2 \\times 2\\sqrt{3} = 4\\sqrt{3}\\)."
},
{
  topic: "Surds - Simplifying",
  level: "Higher",
  question: "Simplify \\(\\sqrt{45} - \\sqrt{5}\\).",
  solution: "\\(3\\sqrt{5} - \\sqrt{5} = 2\\sqrt{5}\\)."
},
{
  topic: "Surds - Simplifying",
  level: "Higher",
  question: "Simplify \\(\\sqrt{20} + \\sqrt{80}\\).",
  solution: "\\(2\\sqrt{5} + 4\\sqrt{5} = 6\\sqrt{5}\\)."
},
{
  topic: "Surds - Simplifying",
  level: "Higher",
  question: "Simplify \\(\\sqrt{32}\\).",
  solution: "\\(\\sqrt{16 \\times 2} = 4\\sqrt{2}\\)."
},
{
  topic: "Surds - Simplifying",
  level: "Higher",
  question: "Simplify \\(3\\sqrt{27}\\).",
  solution: "\\(3 \\times 3\\sqrt{3} = 9\\sqrt{3}\\)."
},
{
  topic: "Surds - Simplifying",
  level: "Higher",
  question: "Simplify \\(\\sqrt{98}\\).",
  solution: "\\(\\sqrt{49 \\times 2} = 7\\sqrt{2}\\)."
},
{
  topic: "Surds - Simplifying",
  level: "Higher",
  question: "Simplify \\(\\sqrt{75} - \\sqrt{3}\\).",
  solution: "\\(5\\sqrt{3} - \\sqrt{3} = 4\\sqrt{3}\\)."
},

// =======================
// Surds – Rationalise the Denominator
// =======================

{
  topic: "Surds - Rationalise the Denominator",
  level: "Higher",
  question: "Rationalise the denominator: \\(\\frac{5}{\\sqrt{3}}\\).",
  solution: "\\(\\frac{5\\sqrt{3}}{3}\\)."
},
{
  topic: "Surds - Rationalise the Denominator",
  level: "Higher",
  question: "Rationalise the denominator: \\(\\frac{2}{\\sqrt{7}}\\).",
  solution: "\\(\\frac{2\\sqrt{7}}{7}\\)."
},
{
  topic: "Surds - Rationalise the Denominator",
  level: "Higher",
  question: "Rationalise the denominator: \\(\\frac{4}{\\sqrt{5}}\\).",
  solution: "\\(\\frac{4\\sqrt{5}}{5}\\)."
},
{
  topic: "Surds - Rationalise the Denominator",
  level: "Higher",
  question: "Rationalise the denominator: \\(\\frac{7}{\\sqrt{2}}\\).",
  solution: "\\(\\frac{7\\sqrt{2}}{2}\\)."
},
{
  topic: "Surds - Rationalise the Denominator",
  level: "Higher",
  question: "Rationalise the denominator: \\(\\frac{3}{2\\sqrt{5}}\\).",
  solution: "\\(\\frac{3\\sqrt{5}}{10}\\)."
},
{
  topic: "Surds - Rationalise the Denominator",
  level: "Higher",
  question: "Rationalise the denominator: \\(\\frac{6}{\\sqrt{3}}\\).",
  solution: "\\(\\frac{6\\sqrt{3}}{3} = 2\\sqrt{3}\\)."
},
{
  topic: "Surds - Rationalise the Denominator",
  level: "Higher",
  question: "Rationalise the denominator: \\(\\frac{1}{\\sqrt{8}}\\).",
  solution: "\\(\\frac{\\sqrt{8}}{8} = \\frac{\\sqrt{2}}{4}\\)."
},
{
  topic: "Surds - Rationalise the Denominator",
  level: "Higher",
  question: "Rationalise the denominator: \\(\\frac{5}{3\\sqrt{2}}\\).",
  solution: "\\(\\frac{5\\sqrt{2}}{6}\\)."
},
{
  topic: "Surds - Rationalise the Denominator",
  level: "Higher",
  question: "Rationalise the denominator: \\(\\frac{9}{\\sqrt{6}}\\).",
  solution: "\\(\\frac{9\\sqrt{6}}{6} = \\frac{3\\sqrt{6}}{2}\\)."
},
{
  topic: "Surds - Rationalise the Denominator",
  level: "Higher",
  question: "Rationalise the denominator: \\(\\frac{2}{5\\sqrt{3}}\\).",
  solution: "\\(\\frac{2\\sqrt{3}}{15}\\)."
},

// =======================
// Surds – Expanding Brackets
// =======================

{
  topic: "Surds - Expanding Brackets",
  level: "Higher",
  question: "Expand and simplify: \\((\\sqrt{5} + 2)(\\sqrt{5} - 2)\\).",
  solution: "\\(5 - 4 = 1\\)."
},
{
  topic: "Surds - Expanding Brackets",
  level: "Higher",
  question: "Expand and simplify: \\((\\sqrt{3} + 1)^2\\).",
  solution: "\\(3 + 2\\sqrt{3} + 1 = 4 + 2\\sqrt{3}\\)."
},
{
  topic: "Surds - Expanding Brackets",
  level: "Higher",
  question: "Expand and simplify: \\((\\sqrt{7} - 3)^2\\).",
  solution: "\\(7 - 6\\sqrt{7} + 9 = 16 - 6\\sqrt{7}\\)."
},
{
  topic: "Surds - Expanding Brackets",
  level: "Higher",
  question: "Expand and simplify: \\((2 + \\sqrt{5})(3 + \\sqrt{5})\\).",
  solution: "\\(6 + 2\\sqrt{5} + 3\\sqrt{5} + 5 = 11 + 5\\sqrt{5}\\)."
},
{
  topic: "Surds - Expanding Brackets",
  level: "Higher",
  question: "Expand and simplify: \\((\\sqrt{2} + 4)(\\sqrt{2} - 1)\\).",
  solution: "\\(2 - \\sqrt{2} + 4\\sqrt{2} - 4 = 3\\sqrt{2} - 2\\)."
},
{
  topic: "Surds - Expanding Brackets",
  level: "Higher",
  question: "Expand and simplify: \\((\\sqrt{3} + 2)(\\sqrt{3} + 5)\\).",
  solution: "\\(3 + 5\\sqrt{3} + 2\\sqrt{3} + 10 = 13 + 7\\sqrt{3}\\)."
},
{
  topic: "Surds - Expanding Brackets",
  level: "Higher",
  question: "Expand and simplify: \\((\\sqrt{6} - 2)(\\sqrt{6} + 3)\\).",
  solution: "\\(6 + 3\\sqrt{6} - 2\\sqrt{6} - 6 = \\sqrt{6}\\)."
},
{
  topic: "Surds - Expanding Brackets",
  level: "Higher",
  question: "Expand and simplify: \\((1 + \\sqrt{2})^2\\).",
  solution: "\\(1 + 2\\sqrt{2} + 2 = 3 + 2\\sqrt{2}\\)."
},
{
  topic: "Surds - Expanding Brackets",
  level: "Higher",
  question: "Expand and simplify: \\((\\sqrt{5} - 1)(\\sqrt{5} + 4)\\).",
  solution: "\\(5 + 4\\sqrt{5} - \\sqrt{5} - 4 = 1 + 3\\sqrt{5}\\)."
},
{
  topic: "Surds - Expanding Brackets",
  level: "Higher",
  question: "Expand and simplify: \\((2\\sqrt{3} + 1)(\\sqrt{3} - 2)\\).",
  solution: "\\(6 - 4\\sqrt{3} + \\sqrt{3} - 2 = 4 - 3\\sqrt{3}\\)."
}
,
    
    {
      topic: "Solving Quadratics",
      level: "Higher",
      question: "Solve \\(x^2 + 3x - 10 = 0\\).",
      solution: "\\((x + 5)(x - 2) = 0\\), so \\(x = -5, 2\\)."
    },
    {
      topic: "Solving Quadratics",
      level: "Higher",
      question: "Solve \\(x^2 - 7x + 10 = 0\\).",
      solution: "\\((x - 5)(x - 2) = 0\\), so \\(x = 5, 2\\)."
    },
    {
      topic: "Solving Quadratics",
      level: "Higher",
      question: "Solve \\(2x^2 - 5x - 3 = 0\\).",
      solution: "Factorise to \\((2x + 1)(x - 3) = 0\\)."
    },
    {
      topic: "Solving Quadratics",
      level: "Higher",
      question: "Solve \\(x^2 = 7x\\).",
      solution: "\\(x(x - 7) = 0\\), so \\(x = 0, 7\\)."
    },
    {
      topic: "Solving Quadratics",
      level: "Higher",
      question: "Solve \\(x^2 - 16 = 0\\).",
      solution: "\\(x = \\pm4\\)."
    },
    
    {
      topic: "Completing the Square",
      level: "Higher",
      question: "Complete the square for \\(x^2 + 6x + 5\\).",
      solution: "\\((x + 3)^2 - 4\\)."
    },
    {
      topic: "Completing the Square",
      level: "Higher",
      question: "Complete the square for \\(x^2 - 4x + 1\\).",
      solution: "\\((x - 2)^2 - 3\\)."
    },
    {
      topic: "Completing the Square",
      level: "Higher",
      question: "Solve \\(x^2 + 6x + 5 = 0\\) by completing the square.",
      solution: "\\((x + 3)^2 = 4\\), so \\(x = -1, -5\\)."
    },
    {
      topic: "Completing the Square",
      level: "Higher",
      question: "Write \\(x^2 + 8x + 7\\) in completed square form.",
      solution: "\\((x + 4)^2 - 9\\)."
    },
    {
      topic: "Completing the Square",
      level: "Higher",
      question: "Find the minimum value of \\(x^2 - 6x + 10\\).",
      solution: "\\((x - 3)^2 + 1\\), minimum value is 1."
    },
    
    {
      topic: "Parallel and Perpendicular Lines",
      level: "Higher",
      question: "Find the gradient of a line perpendicular to \\(y = 2x + 5\\).",
      solution: "The perpendicular gradient is \\(-\\frac{1}{2}\\)."
    },
    {
      topic: "Parallel and Perpendicular Lines",
      level: "Higher",
      question: "Find the equation of a line parallel to \\(y = -3x + 1\\) through \\((2,4)\\).",
      solution: "\\(y = -3x + 10\\)."
    },
    {
      topic: "Parallel and Perpendicular Lines",
      level: "Higher",
      question: "Find the equation of the line perpendicular to \\(y = 4x - 7\\) through \\((1,3)\\).",
      solution: "Gradient is \\(-\\frac{1}{4}\\), equation is \\(y = -\\frac{1}{4}x + \\frac{13}{4}\\)."
    },
    {
      topic: "Parallel and Perpendicular Lines",
      level: "Higher",
      question: "Are the lines \\(y = 2x + 1\\) and \\(2y = 4x - 3\\) parallel?",
      solution: "Both have gradient 2, so yes they are parallel."
    },
    {
      topic: "Parallel and Perpendicular Lines",
      level: "Higher",
      question: "Find the gradient of the line joining \\((2,5)\\) and \\((6,13)\\).",
      solution: "\\(\\frac{13 - 5}{6 - 2} = 2\\)."
    }
  ,
  
  /* =========================
   FOUNDATION
   ========================= */

{
  topic: "Order of Operations",
  level: "Foundation",
  question: "Work out \\(6 + 18 \\div 3 \\times 2\\).",
  solution: "Division and multiplication first: \\(18 \\div 3 = 6\\), \\(6 \\times 2 = 12\\). Then \\(6 + 12 = 18\\)."
},
{
  topic: "Order of Operations",
  level: "Foundation",
  question: "Work out \\(5(4 + 6) - 12 \\div 3\\).",
  solution: "Brackets first: \\(5 \\times 10 = 50\\). Then division: \\(12 \\div 3 = 4\\). Final answer: \\(50 - 4 = 46\\)."
},
{
  topic: "Order of Operations",
  level: "Foundation",
  question: "Calculate \\(20 - 4^2 \\times 2\\).",
  solution: "Indices first: \\(4^2 = 16\\). Then \\(16 \\times 2 = 32\\). Final answer: \\(20 - 32 = -12\\)."
},
{
  topic: "Order of Operations",
  level: "Foundation",
  question: "Work out \\((8 + 4) \\div 2 + 3^2\\).",
  solution: "Brackets first: \\(12 \\div 2 = 6\\). Indices: \\(3^2 = 9\\). Total: \\(6 + 9 = 15\\)."
},
{
  topic: "Order of Operations",
  level: "Foundation",
  question: "Evaluate \\(3 + 24 \\div (6 - 2)\\).",
  solution: "Brackets first: \\(6 - 2 = 4\\). Then \\(24 \\div 4 = 6\\). Final answer: \\(9\\)."
},

{
  topic: "Factors and Multiples",
  level: "Foundation",
  question: "Write down all the factors of 24.",
  solution: "The factors are \\(1,2,3,4,6,8,12,24\\)."
},
{
  topic: "Factors and Multiples",
  level: "Foundation",
  question: "Find the highest common factor (HCF) of 36 and 48.",
  solution: "The HCF is 12."
},
{
  topic: "Factors and Multiples",
  level: "Foundation",
  question: "Find the lowest common multiple (LCM) of 6 and 10.",
  solution: "Multiples of 6: 6,12,18,24,30. Multiples of 10: 10,20,30. LCM is 30."
},
{
  topic: "Factors and Multiples",
  level: "Foundation",
  question: "Is 91 a prime number? Give a reason for your answer.",
  solution: "No, because \\(91 = 7 \\times 13\\)."
},
{
  topic: "Factors and Multiples",
  level: "Foundation",
  question: "Write 60 as a product of its prime factors.",
  solution: "\\(60 = 2^2 \\times 3 \\times 5\\)."
},

{
  topic: "Averages",
  level: "Foundation",
  question: "The numbers are 4, 7, 9, 10 and 10. Find the mean.",
  solution: "Total is 40. Mean is \\(40 \\div 5 = 8\\)."
},
{
  topic: "Averages",
  level: "Foundation",
  question: "The mean of 6 numbers is 12. Five of the numbers total 55. Find the sixth number.",
  solution: "Total is \\(6 \\times 12 = 72\\). Missing number is \\(72 - 55 = 17\\)."
},
{
  topic: "Averages",
  level: "Foundation",
  question: "Find the median of the numbers 3, 7, 8, 11, 14.",
  solution: "The middle value is 8."
},
{
  topic: "Averages",
  level: "Foundation",
  question: "Find the mode of the numbers 2, 5, 5, 6, 7, 7, 7.",
  solution: "The most common value is 7."
},
{
  topic: "Averages",
  level: "Foundation",
  question: "The mean of 4 numbers is 9. Three numbers are 4, 7 and 15. Find the fourth number.",
  solution: "Total is \\(4 \\times 9 = 36\\). Missing number is \\(36 - 26 = 10\\)."
},

{
  topic: "Percentages",
  level: "Foundation",
  question: "Find 15% of £240.",
  solution: "10% is £24, 5% is £12. Total is £36."
},
{
  topic: "Percentages",
  level: "Foundation",
  question: "A jacket costs £80. It is reduced by 25%. Find the sale price.",
  solution: "25% of £80 is £20. Sale price is £60."
},
{
  topic: "Percentages",
  level: "Foundation",
  question: "Increase £120 by 10%.",
  solution: "10% is £12, so new amount is £132."
},
{
  topic: "Percentages",
  level: "Foundation",
  question: "Decrease 500 by 18%.",
  solution: "10% is 50, 8% is 40. Total decrease is 90. New value is 410."
},
{
  topic: "Percentages",
  level: "Foundation",
  question: "What percentage of 200 is 30?",
  solution: "\\(\\frac{30}{200} \\times 100 = 15\\%\\)."
},

{
  topic: "Substitution",
  level: "Foundation",
  question: "Work out \\(3x + 5\\) when \\(x = 4\\).",
  solution: "\\(3(4) + 5 = 17\\)."
},
{
  topic: "Substitution",
  level: "Foundation",
  question: "Find the value of \\(2a^2 - 3a\\) when \\(a = 5\\).",
  solution: "\\(2(25) - 15 = 35\\)."
},
{
  topic: "Substitution",
  level: "Foundation",
  question: "Work out \\(\\frac{y}{2} + 4\\) when \\(y = 10\\).",
  solution: "\\(5 + 4 = 9\\)."
},
{
  topic: "Substitution",
  level: "Foundation",
  question: "Find the value of \\(4p - q\\) when \\(p = 6\\) and \\(q = 7\\).",
  solution: "\\(24 - 7 = 17\\)."
},
{
  topic: "Substitution",
  level: "Foundation",
  question: "Calculate \\(x^2 - 2x\\) when \\(x = 3\\).",
  solution: "\\(9 - 6 = 3\\)."
},

/* =========================
   CROSSOVER
   ========================= */

{
  topic: "Angles in Polygons",
  level: "Crossover",
  question: "Work out the size of each interior angle of a regular hexagon.",
  solution: "Sum is \\((6 - 2) \\times 180 = 720\\). Each angle is \\(120^\\circ\\)."
},
{
  topic: "Angles in Polygons",
  level: "Crossover",
  question: "The interior angle of a regular polygon is \\(150^\\circ\\). How many sides does it have?",
  solution: "Exterior angle is 30°. \\(360 \\div 30 = 12\\) sides."
},
{
  topic: "Angles in Polygons",
  level: "Crossover",
  question: "Find the sum of the interior angles of a nonagon.",
  solution: "\\((9 - 2) \\times 180 = 1260^\\circ\\)."
},
{
  topic: "Angles in Polygons",
  level: "Crossover",
  question: "A regular polygon has 15 sides. Find the size of each interior angle.",
  solution: "Sum is \\(13 \\times 180 = 2340\\). Each angle is \\(156^\\circ\\)."
},
{
  topic: "Angles in Polygons",
  level: "Crossover",
  question: "Find the exterior angle of a regular octagon.",
  solution: "\\(360 \\div 8 = 45^\\circ\\)."
},

{
  topic: "Volume and Surface Area",
  level: "Crossover",
  question: "Find the volume of a cuboid measuring 8 cm by 5 cm by 3 cm.",
  solution: "\\(8 \\times 5 \\times 3 = 120\\text{ cm}^3\\)."
},
{
  topic: "Volume and Surface Area",
  level: "Crossover",
  question: "Calculate the surface area of a cube with side length 4 cm.",
  solution: "\\(6 \\times 4^2 = 96\\text{ cm}^2\\)."
},
{
  topic: "Volume and Surface Area",
  level: "Crossover",
  question: "Find the volume of a cylinder with radius 3 cm and height 10 cm.",
  solution: "\\(\\pi r^2 h = 90\\pi\\text{ cm}^3\\)."
},
{
  topic: "Volume and Surface Area",
  level: "Crossover",
  question: "A sphere has radius 7 cm. Find its surface area.",
  solution: "\\(4\\pi r^2 = 196\\pi\\text{ cm}^2\\)."
},
{
  topic: "Volume and Surface Area",
  level: "Crossover",
  question: "A cone has radius 5 cm and height 12 cm. Find its volume.",
  solution: "\\(\\frac{1}{3}\\pi r^2 h = 100\\pi\\text{ cm}^3\\)."
},

{
  topic: "Probability Trees",
  level: "Crossover",
  question: "A bag contains 3 red and 5 blue counters. Two counters are taken without replacement. Find the probability both are red.",
  solution: "\\(\\frac{3}{8} \\times \\frac{2}{7} = \\frac{3}{28}\\)."
},
{
  topic: "Probability Trees",
  level: "Crossover",
  question: "A biased coin has probability \\(\\frac{2}{3}\\) of landing heads. Find the probability of two heads.",
  solution: "\\(\\frac{2}{3} \\times \\frac{2}{3} = \\frac{4}{9}\\)."
},
{
  topic: "Probability Trees",
  level: "Crossover",
  question: "Two dice are rolled. Find the probability the total is 7.",
  solution: "There are 6 outcomes that give 7 out of 36. Probability is \\(\\frac{1}{6}\\)."
},
{
  topic: "Probability Trees",
  level: "Crossover",
  question: "A bag contains 4 green and 6 yellow balls. One is taken, replaced, then another is taken. Find the probability both are yellow.",
  solution: "\\(\\frac{6}{10} \\times \\frac{6}{10} = \\frac{9}{25}\\)."
},
{
  topic: "Probability Trees",
  level: "Crossover",
  question: "The probability it rains on a day is 0.3. Find the probability it does not rain on two consecutive days.",
  solution: "\\(0.7 \\times 0.7 = 0.49\\)."
},

{
  topic: "Algebraic Fractions",
  level: "Crossover",
  question: "Simplify \\(\\frac{6x}{9}\\).",
  solution: "\\(\\frac{2x}{3}\\)."
},
{
  topic: "Algebraic Fractions",
  level: "Crossover",
  question: "Simplify \\(\\frac{x^2}{4x}\\).",
  solution: "\\(\\frac{x}{4}\\)."
},
{
  topic: "Algebraic Fractions",
  level: "Crossover",
  question: "Simplify \\(\\frac{3x}{5} + \\frac{2x}{5}\\).",
  solution: "\\(x\\)."
},
{
  topic: "Algebraic Fractions",
  level: "Crossover",
  question: "Simplify \\(\\frac{4x}{6} \\times \\frac{3}{2x}\\).",
  solution: "\\(1\\)."
},
{
  topic: "Algebraic Fractions",
  level: "Crossover",
  question: "Simplify \\(\\frac{2x + 4}{2}\\).",
  solution: "\\(x + 2\\)."
},

/* =========================
   HIGHER
   ========================= */

{
  topic: "Direct and Inverse Proportion",
  level: "Higher",
  question: "y is directly proportional to x. When x = 4, y = 10. Find y when x = 7.",
  solution: "y = kx, so k = 2.5. When x = 7, y = 17.5."
},
{
  topic: "Direct and Inverse Proportion",
  level: "Higher",
  question: "y is inversely proportional to x. When x = 5, y = 8. Find y when x = 20.",
  solution: "y = \\frac{k}{x}, so k = 40. y = 2."
},
{
  topic: "Direct and Inverse Proportion",
  level: "Higher",
  question: "y is directly proportional to x². When x = 3, y = 18. Find y when x = 5.",
  solution: "y = kx², so k = 2. y = 50."
},
{
  topic: "Direct and Inverse Proportion",
  level: "Higher",
  question: "y is inversely proportional to x². When x = 2, y = 12. Find y when x = 6.",
  solution: "k = 48, so y = \\frac{48}{36} = \\frac{4}{3}."
},
{
  topic: "Direct and Inverse Proportion",
  level: "Higher",
  question: "The time taken to complete a job is inversely proportional to the number of workers. 6 workers take 10 days. How long will 15 workers take?",
  solution: "Total work = 60 worker-days. Time = 4 days."
},

{
  topic: "Exact Trigonometry Values",
  level: "Higher",
  question: "Find the exact value of \\(\\sin 30^\\circ + \\cos 60^\\circ\\).",
  solution: "\\(\\frac{1}{2} + \\frac{1}{2} = 1\\)."
},
{
  topic: "Exact Trigonometry Values",
  level: "Higher",
  question: "Find the exact value of \\(\\tan 45^\\circ + \\sin 60^\\circ\\).",
  solution: "\\(1 + \\frac{\\sqrt{3}}{2}\\)."
},
{
  topic: "Exact Trigonometry Values",
  level: "Higher",
  question: "Find the exact value of \\(2\\sin 30^\\circ - \\cos 0^\\circ\\).",
  solution: "\\(1 - 1 = 0\\)."
},
{
  topic: "Exact Trigonometry Values",
  level: "Higher",
  question: "Evaluate \\(\\frac{\\sin 60^\\circ}{\\cos 30^\\circ}\\).",
  solution: "Both equal \\(\\frac{\\sqrt{3}}{2}\\), so the answer is 1."
},
{
  topic: "Exact Trigonometry Values",
  level: "Higher",
  question: "Find the exact value of \\(\\sin 45^\\circ \\times \\cos 45^\\circ\\).",
  solution: "\\(\\frac{1}{2}\\)."
},

{
  topic: "Recurring Decimals to Fractions",
  level: "Higher",
  question: "Write \\(0.\\overline{3}\\) as a fraction.",
  solution: "\\(\\frac{1}{3}\\)."
},
{
  topic: "Recurring Decimals to Fractions",
  level: "Higher",
  question: "Write \\(0.\\overline{6}\\) as a fraction.",
  solution: "\\(\\frac{2}{3}\\)."
},
{
  topic: "Recurring Decimals to Fractions",
  level: "Higher",
  question: "Write \\(0.1\\overline{2}\\) as a fraction.",
  solution: "\\(\\frac{11}{90}\\)."
},
{
  topic: "Recurring Decimals to Fractions",
  level: "Higher",
  question: "Write \\(0.\\overline{45}\\) as a fraction.",
  solution: "\\(\\frac{45}{99} = \\frac{5}{11}\\)."
},
{
  topic: "Recurring Decimals to Fractions",
  level: "Higher",
  question: "Write \\(0.2\\overline{7}\\) as a fraction.",
  solution: "\\(\\frac{25}{90} = \\frac{5}{18}\\)."
},

{
  topic: "Expanding Triple Brackets",
  level: "Higher",
  question: "Expand \\((x + 1)(x + 2)(x + 3)\\).",
  solution: "\\(x^3 + 6x^2 + 11x + 6\\)."
},
{
  topic: "Expanding Triple Brackets",
  level: "Higher",
  question: "Expand \\((x - 2)(x + 1)(x + 4)\\).",
  solution: "\\(x^3 + 3x^2 - 6x - 8\\)."
},
{
  topic: "Expanding Triple Brackets",
  level: "Higher",
  question: "Expand \\((2x + 1)(x - 3)(x + 2)\\).",
  solution: "\\(2x^3 - 5x^2 - 11x - 6\\)."
},
{
  topic: "Expanding Triple Brackets",
  level: "Higher",
  question: "Expand \\((x - 1)^2(x + 2)\\).",
  solution: "\\(x^3 - 3x + 2\\)."
},
{
  topic: "Expanding Triple Brackets",
  level: "Higher",
  question: "Expand \\((x + 3)^2(x - 1)\\).",
  solution: "\\(x^3 + 5x^2 + 3x - 9\\)."
},

{
  topic: "Rationalising the Denominator of Surds",
  level: "Higher",
  question: "Rationalise \\(\\frac{5}{\\sqrt{3}}\\).",
  solution: "\\(\\frac{5\\sqrt{3}}{3}\\)."
},
{
  topic: "Rationalising the Denominator of Surds",
  level: "Higher",
  question: "Rationalise \\(\\frac{4}{2\\sqrt{5}}\\).",
  solution: "\\(\\frac{2\\sqrt{5}}{5}\\)."
},
{
  topic: "Rationalising the Denominator of Surds",
  level: "Higher",
  question: "Rationalise \\(\\frac{3}{\\sqrt{7}}\\).",
  solution: "\\(\\frac{3\\sqrt{7}}{7}\\)."
},
{
  topic: "Rationalising the Denominator of Surds",
  level: "Higher",
  question: "Rationalise \\(\\frac{6}{\\sqrt{2}}\\).",
  solution: "\\(3\\sqrt{2}\\)."
},
{
  topic: "Rationalising the Denominator of Surds",
  level: "Higher",
  question: "Rationalise \\(\\frac{7}{\\sqrt{5}}\\).",
  solution: "\\(\\frac{7\\sqrt{5}}{5}\\)."
}

    ];
    
